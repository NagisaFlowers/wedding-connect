    ...
    validateField(field) {
        ...
        // 2. Nombre
            ...
            if (!soloLetras.test(field.value)) {
                ...
                errorMessage = 'El nombre no puede tener más de 40 caracteres.';
            }
        }
        // 3. Correo
            ...
            if (!emailValido.test(field.value)) {
                ...
                errorMessage = 'El correo no puede exceder los 60 caracteres.';
            }
        }
        // 4. Teléfono
        ...
        // 5. Fecha
            ...
            console.log("Validando fecha - valor del input:", field.value);
            
            // Dividir la fecha en partes para evitar problemas de zona horaria
            const partes = field.value.split('-');
            const año = parseInt(partes[0]);
            const mes = parseInt(partes[1]) - 1; // Los meses en JS son 0-11
            const dia = parseInt(partes[2]);
            
            // Crear fecha seleccionada en UTC (sin zona horaria)
            const selectedDate = new Date(Date.UTC(año, mes, dia));
            ...
            console.log("Fecha seleccionada (UTC ms):", selectedDate.getTime());
            console.log("Fecha hoy (UTC ms):", today.getTime());
            
            if (selectedDate < today) {
                errorMessage = 'La fecha del evento debe ser hoy o una fecha futura.';
                console.log("ERROR: fecha pasada");
            } else {
...
// Limpiar localStorage al hacer reset del formulario
const resetButton = document.querySelector('button[type="reset"]');
if (resetButton && registroValidator.form) {
    resetButton.addEventListener('click', function(e) {
        // Pequeño retraso para permitir que el reset ocurra primero
        setTimeout(() => {
            localStorage.removeItem('weddingConnectFormData');
